...
class AddDebitCardScreen extends StatefulWidget {
  ...
  AddDebitCardScreen(this.paymentParameter, [this.selectedcard]);

  bool isEdit = true;

  @override
  _AddDebitCardScreenState createState() =>
      _AddDebitCardScreenState(this.paymentParameter);
}
...
class AddDebitCardScreen extends StatefulWidget {
  ...
  AddDebitCardScreen(this.paymentParameter);
...
/*  TextEditingController cardnameController = TextEditingController();
  ...
  var trip_number,customer_id,scooter_id,payment_id,subscription_id,ride_start,ride_mins,total_ride_amt,unlock_charge,sub_total,vat_charge,grand_total,scootertripdetails;
  String mobile_test = '';
...
/* adddebit() async {
    ...
    var data = json.encode({
      ...
      "payment_method": "debit",
      "card_holder_name": cardnameController.text,
      "card_number": cardnumberController1.text + cardnumberController2.text +
          cardnumberController3.text + cardnumberController4.text,
      "expire_year": expiryyearController.text,
      "expire_month": expirymonthController.text,
      "cvv": cvvController.text,
      "amount": 100 */ /*
 */
/*paymentParameter['grantTotal']*/ /*
 */
/*,
    });
    print(data);
    final response = await http.post(
        baseurl + 'api_payments', headers: {'authorization': basicAuth},
        body: data);
    print(response.body);
    if (response.statusCode == 200) {
      var jsonResponse = json.decode(response.body);
  ...
  paymentstatus() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonResponse['status'] == 'Success') {
        ...
        _showMessageInScaffold(Color(0xff00DD00), jsonResponse['message']);
        Navigator.of(context).pushAndRemoveUntil(
            MaterialPageRoute(builder: (BuildContext context) => RidePrepaidScreen(scootertripdetails)),
            (Route<dynamic> route) => false);
      } else if (jsonResponse['status'] == 'Error') {
        _showMessageInScaffold(Color(0xff00DD00), jsonResponse['message']);
        Navigator.of(context).pushAndRemoveUntil(
